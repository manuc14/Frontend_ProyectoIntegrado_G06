...
export class RegisterComponent {
  ...
  loadAvatars() {
    ...
    this.api.getAvatars().subscribe({
      next: (response) => {
        // La API devuelve un objeto con la lista de avatares y el avatar por defecto
        this.avatars = (response && (response as any).avatars) ? (response as any).avatars : [];
        this.defaultAvatar = (response && (response as any).defaultAvatar) ? (response as any).defaultAvatar : '';
